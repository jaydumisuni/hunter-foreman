...
<head>
  ...
  <style>
    :root { color-scheme: dark; --bg: #0b0d11; --panel: #151923; --line: #293140; --text: #f4f7fb; --muted: #a7b0c0; --accent: #8dffcf; --warn: #ffd166; }
    * { box-sizing: border-box; }
    ...
    h1 { font-size: clamp(36px, 6vw, 70px); margin: 8px 0; letter-spacing: -0.06em; }
    h2 { margin: 0 0 12px; }
    p { color: var(--muted); line-height: 1.55; }
    .tag { display: inline-block; padding: 7px 11px; border-radius: 999px; background: #1d2430; border: 1px solid var(--line); color: var(--accent); font-weight: 700; font-size: 13px; }
    .grid { display: grid; grid-template-columns: 0.85fr 1.15fr; gap: 18px; }
    ...
    label { display: block; font-size: 13px; font-weight: 800; color: #dbe7f6; margin: 13px 0 7px; }
    ...
    button { margin-top: 14px; background: linear-gradient(135deg, #8dffcf, #9bb7ff); color: #081015; font-weight: 900; cursor: pointer; border: 0; }
    .task { border: 1px solid var(--line); border-radius: 18px; padding: 16px; margin-top: 14px; background: #10151f; }
    .row { display: flex; gap: 8px; flex-wrap: wrap; margin: 10px 0; }
    .pill { display: inline-flex; padding: 6px 9px; border-radius: 999px; background: #202839; color: #dce8f7; font-size: 12px; font-weight: 800; }
    .warn { color: var(--warn); }
...
<body>
  <main>
    <header>
      ...
      <div class="card" style="max-width:360px"><strong>Public-safe demo</strong><p>No private Hunter Core, no repair modules, no secrets, no client data.</p></div>
    </header>
    <section class="grid">
      <div class="card">
        <h2>ROSE Intake</h2>
  ...
  <script>
    ...
    function renderTasks(tasks){
      ...
      tasksEl.innerHTML = tasks.map(task => `
        <article class="task">
          <div class="row"><span class="pill">${task.id}</span><span class="pill">${task.workflow.label}</span><span class="pill">Confidence: ${Math.round(task.confidence * 100)}%</span></div>
